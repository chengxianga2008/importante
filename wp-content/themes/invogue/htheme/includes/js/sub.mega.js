...
jQuery(function(){
    ...
    //SET THE HEADER
    htheme_set_header('Mega Menu');

    //SET DATA
    htheme_set_data();

    //SET DATA
    jQuery(global_options.settings.megamenu.fonts).each(function(index, element){
        //GET HTML
        jQuery('.htheme_load_mega_fonts').append(htheme_get_font_html(index, element.label, element.slug));
        //SET FONTS
        var fonts = [ 'family_'+index ];
        htheme_fonts_options(fonts);
        //SET DATA
        htheme_set_font_data(index);
        //UPDATE DATA
        htheme_update_font_data(index);
    });
...
function htheme_set_data(){
    ...
    var _enable = global_options.settings.megamenu.enable;

    if(_enable){
        if(jQuery('#enable').val() == _enable){
...
function htheme_update_data(){
    ...
    var _enable = jQuery('#enable');

    jQuery(_enable).on('change', function(){
        jQuery(this).prop('checked') ? global_options.settings.megamenu.enable = jQuery(this).val() : global_options.settings.megamenu.enable = false;
...
function htheme_set_weight_data(index){
    ...
    if(global_options.settings.megamenu.fonts[index].weight){
        jQuery('#weight_'+index+' option').each(function(idx, element) {
            if(jQuery(this).val() == global_options.settings.megamenu.fonts[index].weight){
...
function htheme_set_font_data(index){

    //SET DATA
    if(global_options.settings.megamenu.fonts[index].family){
        jQuery('#family_'+index+' option').each(function(idx, element) {
            if(jQuery(this).val() == global_options.settings.megamenu.fonts[index].family){
                jQuery(this).attr('selected', 'selected')
            }
        });
    }

    if(global_options.settings.megamenu.fonts[index].size){
        jQuery('#size_'+index+' option').each(function(idx, element) {
            if(jQuery(this).val() == global_options.settings.megamenu.fonts[index].size){
                jQuery(this).attr('selected', 'selected')
            }
        });
    }

    if(global_options.settings.megamenu.fonts[index].lineHeight){
        jQuery('#lineHeight_'+index+' option').each(function(idx, element) {
            if(jQuery(this).val() == global_options.settings.megamenu.fonts[index].lineHeight){
                jQuery(this).attr('selected', 'selected')
            }
        });
    }

    if(global_options.settings.megamenu.fonts[index].transform){
        jQuery('#transform_'+index+' option').each(function(idx, element) {
            if(jQuery(this).val() == global_options.settings.megamenu.fonts[index].transform){
                jQuery(this).attr('selected', 'selected')
            }
        });
    }

    if(global_options.settings.megamenu.fonts[index].spacing){
        jQuery('#spacing_'+index+' option').each(function(idx, element) {
            if(jQuery(this).val() == global_options.settings.megamenu.fonts[index].spacing){
                jQuery(this).attr('selected', 'selected')
            }
        });
    }

    if(global_options.settings.megamenu.fonts[index].color){
        jQuery('#color_'+index+' option').each(function(idx, element) {
            if(jQuery(this).val() == global_options.settings.megamenu.fonts[index].color){
                jQuery(this).attr('selected', 'selected')
            }
        });
    }
...
//UPDATE DATA
function htheme_update_font_data(index){

    //UPDATE - BODY
    jQuery('#family_'+index).on('change', function(){
        global_options.settings.megamenu.fonts[index].family = jQuery(this).children('option:selected').val();
        htheme_flag_save(true);
    });

    jQuery('#size_'+index).on('change', function(){
        global_options.settings.megamenu.fonts[index].size = jQuery(this).children('option:selected').val();
        htheme_flag_save(true);
    });

    jQuery('#lineHeight_'+index).on('change', function(){
        global_options.settings.megamenu.fonts[index].lineHeight = jQuery(this).children('option:selected').val();
        htheme_flag_save(true);
    });

    jQuery('#weight_'+index).on('change', function(){
        global_options.settings.megamenu.fonts[index].weight = jQuery(this).children('option:selected').val();
        htheme_flag_save(true);
    });

    jQuery('#transform_'+index).on('change', function(){
        global_options.settings.megamenu.fonts[index].transform = jQuery(this).children('option:selected').val();
        htheme_flag_save(true);
    });

    jQuery('#spacing_'+index).on('change', function(){
        global_options.settings.megamenu.fonts[index].spacing = jQuery(this).children('option:selected').val();
        htheme_flag_save(true);
    });

    jQuery('#color_'+index).on('change', function(){
        global_options.settings.megamenu.fonts[index].color = jQuery(this).children('option:selected').val();
        htheme_flag_save(true);
    });
